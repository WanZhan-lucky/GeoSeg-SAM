...
#feature enhance mask decoder with skip connection 
import torch
...
class DecoderWMA(nn.Module):
    """
    Decoder 端的轻量 WMA：
    - 输入: [B, C, H, W]，比如 [B, 32, 256, 256]
    - 输出: 同尺寸 [B, C, H, W]
    """
    def __init__(self, dim: int):
        ...
        self.dw5 = nn.Conv2d(dim, dim, kernel_size=5, padding=2, groups=dim)
    ...
    def forward(self, x):
        ...
        y3 = self.dw3(x)
        y5 = self.dw5(x)
        y  = self.pw(self.act(y3 + y5))
        return x + y    # 残差连接
class LocalRefine(nn.Module):
    def __init__(self, dim=32):
        super().__init__()
        self.conv = nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=1)
...
class MiniASPP(nn.Module):
    def __init__(self, dim=32, dilations=[1, 3, 6]):
        super().__init__()
        self.branches = nn.ModuleList([
            nn.Conv2d(dim, dim, kernel_size=3, padding=d, dilation=d)
            for d in dilations
        ])
        self.project = nn.Conv2d(dim * len(dilations), dim, kernel_size=1)
...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.cls_upscaling= nn.Sequential(
            ...
            LayerNorm2d(transformer_dim // 4),
            activation(),
            nn.Conv2d(transformer_dim // 4, transformer_dim * self.num_classes // 8, kernel_size=7, stride=2,padding=3),
            activation(),
        )


        self.use_pyramid = True
        self.C_ = transformer_dim // 8  # 32
        # 把每个 pyramid feat: (B,256,H,W) -> (B,32,H,W)
        self.pyr_projs = nn.ModuleList([nn.Conv2d(transformer_dim, self.C_, 1, bias=False) for _ in range(4)])
        self.pyr_norms = nn.ModuleList([LayerNorm2d(self.C_) for _ in range(4)])
        self.pyr_gammas = nn.Parameter(torch.zeros(4))  # 从0起步最稳
    ...
    def forward(
        ...
        pyramid_embeddings:Optional[List],
        ...
        masks, iou_pred = self.predict_masks(
            ...
            pyramid_embeddings=pyramid_embeddings,
            image_pe=image_pe,
    ...
    def predict_masks(
        ...
        return masks, iou_pred ##(1,13,256,256)
